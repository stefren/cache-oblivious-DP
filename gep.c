...
uint64_t* get_entry_addr(dp_matrix_t* X, uint64_t i, uint64_t j) {
  if (i > X->width || j > X->height) {
    return NULL;  // return NULL if the entry is out of range
  }
  return X->entries + j*X->width + i;
}
     ...
     for (uint64_t i = i_1; i <= i_2; i++) {
       for (uint64_t j = j_1; j <= j_2; j++) {
         if (update_exists_f(X->width, i, i, j, j, k, k)) {
           uint64_t* x = get_entry_addr(X, i, j);
...
//  printf("%lld %lld %lld %lld %lld %lld\n", i_1, i_2, j_1, j_2, k_1, k_2);
  ...
  if (k_2 - k_1 < CACHE_SIZE) { // base case
    iterative(X, i_1, i_2, j_1, j_2, k_1, k_2, update_f, update_exists_f);
  } else {
